...
| **Карта** | Банкоматы, кафе, рестораны, отели, аптеки — из собственной базы, а не живым запросом наружу. Маршруты пешком и на авто, свои точки |
...
- **Карты** — OpenStreetMap, собственный сервер маршрутизации (пешие и авто),
  своя база мест: точки импортированы из выгрузки OSM и лежат рядом с бэкендом

## Стадия

Работающий продукт: **80 сборок**, бэкенд в production, 4 языка интерфейса,
собственная база мест — **12 313 точек** по стране плюс **39 достопримечательностей**
с описаниями и проверенными координатами.

Пользователей — 15, выручки нет. Продукт есть, рынка ещё нет —
поэтому заявка в инкубацию, а не в акселерацию.

## Рынок

По данным Госкомстата, в 2025 году Узбекистан посетили **11,7 млн** иностранных
гостей — рост 46,8 % за год, экспорт туристических услуг 4,8 млрд долларов.

Поток неоднороден, и продукт это учитывает. Массовому потоку из соседних стран
(Киргизия 3,3 млн, Таджикистан 2,7 млн, Россия 984 тыс.) языковой барьер почти
не мешает — им нужны карта, маршруты, курс валют и местный контекст. Дальним
рынкам (Китай 279 тыс., Европа, Корея, Япония) нужен прежде всего переводчик:
за пределами отелей английский почти не встречается. Разные модули приложения
закрывают разные сегменты.

### Мы попадаем в государственную повестку

Государство развивает концепцию **«смарт-туризма»** — цифровой экосистемы,
которая ведёт туриста от планирования поездки до бытовых задач внутри страны:
мультиязычные гиды, интерактивная карта достопримечательностей, электронные
сервисы. Направление закреплено указом Президента **УП-5611 от 5 января
2019 года** — тем самым, что учредил координационный совет по
государственно-частному партнёрству и цифровизации в сфере туризма — и
стратегией развития туризма до 2030 года.

SilkSafar делает ровно это, только со стороны частной разработки.
...
backend/
  ...
  attractions.py            поиск мест: кириллица и латиница как одна строка,
                            сортировка по расстоянию от туриста
  import_osm_pois.py        импорт 12 313 точек из выгрузки OSM в свою базу
...
mobile/
  ...
  hooks/use-offline-models.ts  скачивание языковых пакетов: ждём появления
                            пакета, а не ответа системы
  utils/location.ts         единая точка входа в геолокацию
  utils/taxi.ts             открытие Яндекс Go без ожидания GPS
...
data/
  attractions_tashkent.json достопримечательности с описаниями, часами работы
                            и координатами, выверенными по OpenStreetMap
```
